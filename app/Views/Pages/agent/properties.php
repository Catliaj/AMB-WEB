...
<body>
  ...
  <div class="container mt-5 pt-5">
    <div class="d-flex justify-content-between align-items-center mb-3">
      <h4 class="fw-bold">Assigned Properties</h4>
      <div>
        <button class="filter-btn" onclick="filterHouses('all')">Filter By</button>
...
<!-- ✅ Edit Property Modal (Image + Availability) -->
    ...
    list.forEach((h, i) => {
      ...
      const card = `
        <div class="col-md-3 col-sm-6">
          <div class="card house-card p-4 shadow-sm animate__animated animate__fadeInUp" 
               style="animation-delay: ${delay}s;">
            <img src="${h.img}" alt="${h.name}">
            <div class="house-info">
              <h6 class="house-title">${h.name}</h6>
              <p>Location: ${h.location}</p>
              <p>Availability: <span class="availability ${h.status === 'Occupied' ? 'occupied' : ''}">${h.status}</span></p>
              <p>Floor: ${h.floor}</p>
              <p>Lot Area: ${h.lot}</p>
              <p>Bedrooms: ${h.bed}</p>
              <div class="d-flex justify-content-between align-items-center mt-2">
                ...
                <button class="btn-view btn-sm">View</button>
              </div>
            </div>
          </div>
        </div>`;
