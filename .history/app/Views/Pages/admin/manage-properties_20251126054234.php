        ...
        <?php
          ...
          ?>
              <tr>
                  ...
                  <td class="actions">
                      <button class="action-btn" data-view="<?= $property['PropertyID']; ?>">
                          <i data-lucide="eye"></i>
                      </button>
                      <button class="action-btn" data-edit="<?= $property['PropertyID']; ?>">
                          <i data-lucide="edit-2"></i>
                      </button>
                      <button class="action-btn danger" data-del="<?= $property['PropertyID']; ?>">
...
// Initialize Bootstrap modal for delete
...
document.getElementById("confirmDeleteBtn").addEventListener("click", () => {
  ...
  function renderTable(data = properties) {
    ...
    data.forEach((p) => {
      ...
      row.innerHTML = `
        ...
        <td class="actions">
          <button class="action-btn" data-view="${p.PropertyID}">
            <i data-lucide='eye'></i>
          </button>
          <button class="action-btn" data-edit="${p.PropertyID}">
            <i data-lucide='edit-2'></i>
          </button>
          <button class="action-btn danger" data-del="${p.PropertyID}">
            <i data-lucide='trash-2'></i>
          </button>
  ...
  function applyFilters() {
    ...
    // Add Property button - reset form and show modal for adding new property
    ...
    btnAddProperty.addEventListener("click", () => {
      ...
      document.getElementById('saveProperty').textContent = 'Save';
      document.getElementById('propertyForm').action = '<?= base_url("admin/property/store-property") ?>';
  ...
  document.querySelector('#propertyTable').addEventListener('click', e => {
    const btn = e.target.closest('button');
    if (!btn) return;

    if (btn.dataset.view) {
      viewProperty(btn.dataset.view);
    } else if (btn.dataset.edit) {
      editProperty(btn.dataset.edit);
...
function editProperty(propertyID) {
   // Edit property - fetch existing data and prefill modal for updating
   fetch(`/admin/getProperty/${propertyID}`)
     .then(res => res.json())
     .then(data => {
       if (!data) return console.error('No data received');
       ...
       document.getElementById('newType').value = data.Property_Type;
       ...
       document.getElementById('newSize').value = data.Size || '';
       document.getElementById('newBedrooms').value = data.Bedrooms || '';
       document.getElementById('newBathrooms').value = data.Bathrooms || '';
       document.getElementById('newParking').value = data.Parking_Spaces || '';
       document.getElementById('newCorporation').value = data.Corporation || '';
       document.getElementById('newAgent').value = data.agent_assigned || '';

       // Change modal to edit mode
       document.getElementById('modalTitle').textContent = 'Edit Property';
       document.getElementById('saveProperty').textContent = 'Update';
       document.getElementById('propertyForm').action = '<?= base_url("admin/property/update-property") ?>';
