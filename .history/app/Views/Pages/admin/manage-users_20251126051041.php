...
document.addEventListener("DOMContentLoaded", () => {
  ...
  userTable.addEventListener('click', e => {
    ...
    // Toggle online/offline - update status via AJAX
    if (toggle) {
      const i = toggle.dataset.index;
      const newStatus = users[i].status === 'Online' ? 'Offline' : 'Online';
      // AJAX to update status
      fetch('<?= base_url("/admin/update-user-status") ?>', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ UserID: users[i].UserID, status: newStatus })
      })
      .then(res => res.json())
      .then(data => {
        if (data.success) {
